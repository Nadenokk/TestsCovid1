...
class CovidResearchByInstitutionNewBySections(unittest.TestCase):
    ...
    def test_covid_research_by_institution_new_by_sections(self):
        ...
        driver.find_element_by_id("reportsForm:j_idt75:7:j_idt77").click()
        ...
        assert (driver.find_element_by_id("dataForm:j_idt85").text == "01.08.2021 00:00 - 16.08.2021 23:59")
        assert (driver.find_element_by_id("dataForm:j_idt87").text == "Определение наличия РНК SARS-CoV-2")
        assert (driver.find_element_by_id(
            "dataForm:j_idt89").text == "Набор реагентов для выявления РНК коронавируса SARS-CoV-2 методом ОТ-ПЦР в режиме реального времени \"РеалБест РНК SARS-CoV-2\"")
        assert (driver.find_element_by_id("dataForm:j_idt147").text == "1")
        ...
        assert (str1 == "Учреждение, направившее материал: Администрация Адмиралтейского района Санкт-Петербурга")
